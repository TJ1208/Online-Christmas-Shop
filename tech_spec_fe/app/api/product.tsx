...
export const getProduct = async (id: string): Promise<ProductModel> => {
    const response = await fetch(`http://localhost:5000https://techspecbe.azurewebsites.net/product/${id}`, {
        next: {
            revalidate: 60
        }
    })
...
export const addProductToImage = async (productImage: ProductImageModel): Promise<ProductImageModel> => {
    const response = await fetch(`http://localhost:5000https://techspecbe.azurewebsites.net/product_image`, {
        method: 'POST',
...
export const deleteProductToImage = async(productId: number, imageId: number): Promise<string> => {
    const response = await fetch(`http://localhost:5000https://techspecbe.azurewebsites.net/product_image/${productId}/${imageId}`, {
        method: 'DELETE',
        headers: {
            'Content-type': 'application/json; charset=UTF-8'
        }
