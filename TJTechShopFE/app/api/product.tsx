...
export const getAllProducts = async (): Promise<ProductModel[]> => {
    const response = await fetch(`https://tjtechbe.azurewebsites.net/product`, {
        next: {
            revalidate: 0
        }
    })
...
export const getProduct = async (id: string): Promise<ProductModel> => {
    const response = await fetch(`https://tjtechbe.azurewebsites.net/product/${id}`, {
        next: {
            revalidate: 60
        }
    })
...
export const addProductToImage = async (productImage: ProductImageModel): Promise<ProductImageModel> => {
    const response = await fetch(`https://tjtechbe.azurewebsites.net/product_image`, {
        method: 'POST',
...
export const deleteProductToImage = async(productId: number, imageId: number): Promise<string> => {
    const response = await fetch(`https://tjtechbe.azurewebsites.net/product_image/${productId}/${imageId}`, {
        method: 'DELETE',
        headers: {
            'Content-type': 'application/json; charset=UTF-8'
        }
